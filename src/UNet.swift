...
func SelfAttention(k: Int, h: Int, b: Int, hw: Int) -> Model {
  ...
  let keys = tokeys(x).reshaped([b, hw, h, k]).to(.Float32).permuted(0, 2, 1, 3)
  let queries = ((1.0 / Float(k).squareRoot()) * toqueries(x)).reshaped([b, hw, h, k]).to(.Float32)
    .permuted(0, 2, 1, 3)
  let values = tovalues(x).reshaped([b, hw, h, k]).permuted(0, 2, 1, 3)
  var dot = Matmul(transposeB: (2, 3))(queries, keys)
  dot = dot.reshaped([b * h * hw, hw])
  dot = dot.softmax()
  dot = dot.reshaped([b, h, hw, hw]).to(of: values)
  var out = dot * values
...
func CrossAttention(k: Int, h: Int, b: Int, hw: Int, t: Int) -> Model {
  ...
  let toqueries = Dense(count: k * h, noBias: true)
  let tovalues = Dense(count: k * h, noBias: true)
  let keys = tokeys(c).reshaped([b, t, h, k]).to(.Float32).permuted(0, 2, 1, 3)
  let queries = ((1.0 / Float(k).squareRoot()) * toqueries(x)).reshaped([b, hw, h, k]).to(.Float32)
    .permuted(0, 2, 1, 3)
  ...
  dot = dot.softmax()
  dot = dot.reshaped([b, h, hw, t]).to(of: values)
  var out = dot * values
  out = out.reshaped([b, h, hw, k]).transposed(1, 2).reshaped([b, hw, h * k])
  let unifyheads = Dense(count: k * h)
  out = unifyheads(out)
