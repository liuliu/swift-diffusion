...
let path = "segmind/SSD-1B"  // model card
let prompt =
  "hyperrealistic glamour portrait of an old weary wizard surrounded by elemental magic, arcane, freckles, skin pores, pores, velus hair, macro, extreme details, looking at viewer"
let negative_prompt =
  "sketch, cartoon, drawing, anime:1.4, low quality, jpeg artifacts, ugly, duplicate, morbid, mutilated, extra fingers, mutated hands, poorly drawn hands, poorly drawn face, mutation, deformed, blurry, dehydrated, bad anatomy, bad proportions, extra limbs, cloned face, disfigured, gross proportions"
...
// let img = pipe(prompt: prompt,negative_prompt: negative_prompt, num_inference_steps: 50, guidance_scale: 9, num_images_per_prompt: 1).images[0]
// img.save("/home/liu/workspace/swift-diffusion/image.png")
...
func UNetXL(
  ...
  inputAttentionRes: KeyValuePairs<Int, [Int]>, outputAttentionRes: KeyValuePairs<Int, [Int]>
) -> ((PythonObject) -> Void, Model) {
  let x = Input()
  let t_emb = Input()
  let y = Input()
  ...
  let (middleReader, middleBlock) = MiddleBlock(
    ...
    embeddingSize: 77, x: out, emb: emb)
  out = middleBlock
  ...
  return (reader, Model([x, t_emb, y] + inputKVs + outputKVs, [out]))
}
...
let (readerFixed, unetFixed) = UNetXLFixed(
  ...
  inputAttentionRes: [2: [2, 2], 4: [4, 4]], outputAttentionRes: [2: [2, 1, 1], 4: [4, 4, 10]])
...
graph.withNoGrad {
  ...
  debugPrint(attnOut)
  graph.openStore("/home/liu/workspace/swift-diffusion/ssd_1b_f32.ckpt") {
