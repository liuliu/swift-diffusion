...
func MMDiT(b: Int, h: Int, w: Int) -> ((PythonObject) -> Void, Model) {
  ...
  let shift = Dense(count: 3072, name: "ada_ln_0")
  let scale = Dense(count: 3072, name: "ada_ln_1")
  ...
  */
  let reader: (PythonObject) -> Void = { state_dict in
    ...
    let final_layer_linear_weight = state_dict["proj_out.weight"].to(
      torch.float
    ).cpu().numpy()
    let final_layer_linear_bias = state_dict["proj_out.bias"].to(
      torch.float
    ).cpu().numpy()
  ...
  return (reader, Model([x, t, contextIn], [context, out]))
}
