...
enum Q21VisionConfig {
  ...
  static let outputSize = 4_096
  static let deepStackLayers = [8, 16, 24]
}

typealias Q21VisionGrid = (t: Int, h: Int, w: Int)

func q21VisionTokenCount(_ grids: [Q21VisionGrid]) -> Int {
...
func q21VisionPositions(_ grids: [Q21VisionGrid]) -> (
  ids: Tensor<Int32>, weights: Tensor<Float>, rotary: Tensor<Float>
) {
  let count = q21VisionTokenCount(grids)
  let side = Q21VisionConfig.positionSide
  let half = Q21VisionConfig.headDim / 2
  var ids = Tensor<Int32>(.CPU, .C(4 * count))
  var weights = Tensor<Float>(.CPU, .C(4 * count))
  var rotary = Tensor<Float>(.CPU, .NHWC(1, count, 1, Q21VisionConfig.headDim))
  var token = 0
  for grid in grids {
    for _ in 0..<grid.t {
...
func Q21VisionAttention<T: TensorNumeric>(
  _ type: T.Type, prefix: String, grids: [Q21VisionGrid]
) -> (Model, [Q21VisionBinding]) {
  let x = Input()
  let rotary = Input()
  let count = q21VisionTokenCount(grids)
  let width = Q21VisionConfig.hiddenSize
  let heads = Q21VisionConfig.heads
  let dim = Q21VisionConfig.headDim
...
func Q21VisionBlock<T: TensorNumeric>(
  _ type: T.Type, layer: Int, grids: [Q21VisionGrid]
) -> (Model, [Q21VisionBinding]) {
  ...
  let (attention, bindings) = Q21VisionAttention(type, prefix: "\(prefix).attn", grids: grids)
  var out = x + attention(norm1(x), rotary)
  let fc1 = Dense(count: Q21VisionConfig.intermediateSize, name: "mlp_fc1")
  let fc2 = Dense(count: Q21VisionConfig.hiddenSize, name: "mlp_fc2")
...
func Q21VisionMerger(prefix: String, name: String, count: Int, postShuffleNorm: Bool) -> (
  Model, [Q21VisionBinding]
) {
  let x = Input()
  let norm = LayerNorm(
    epsilon: 1e-6, axis: [1], name: postShuffleNorm ? "\(name)_norm" : "norm_out")
  let shape = TensorShapeFormat.NC(count / 4, Q21VisionConfig.mergedSize)
  ...
  let fc1 = Dense(count: Q21VisionConfig.mergedSize, name: "\(name)_mlp_0")
  let fc2 = Dense(count: Q21VisionConfig.outputSize, name: "\(name)_mlp_1")
  return (
...
func Q21VisionModel<T: TensorNumeric>(
  _ type: T.Type, grids: [Q21VisionGrid]
) -> (Model, [Q21VisionBinding]) {
  let count = q21VisionTokenCount(grids)
  let width = Q21VisionConfig.hiddenSize
  let patches = Input()
  ...
  let position = Embedding(
    T.self, vocabularySize: Q21VisionConfig.positionEmbeddings,
  ...
  for layer in 0..<Q21VisionConfig.layers {
    let (block, blockBindings) = Q21VisionBlock(type, layer: layer, grids: grids)
    out = block(out, rotary)
    bindings += blockBindings
    if let index = Q21VisionConfig.deepStackLayers.firstIndex(of: layer) {
      let (merger, mergerBindings) = Q21VisionMerger(
